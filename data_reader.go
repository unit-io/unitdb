...
func newDataReader(dt *_DataTable) *_DataReader {
	return &_DataReader{dataTable: dt}
}
...
func (r *_DataReader) readMessage(s _Slot) ([]byte, []byte, error) {
	...
	message, err := r.dataTable.file.Slice(s.msgOffset, s.msgOffset+int64(s.mSize()))
	if err != nil {
		return nil, nil, err
	}
...
func (r *_DataReader) readTopic(s _Slot) ([]byte, error) {
	...
	return r.dataTable.file.Slice(s.msgOffset+int64(idSize), s.msgOffset+int64(s.topicSize)+int64(idSize))
}
