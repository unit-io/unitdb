...
func recoverSplitCrash(db *DB) error {
	...
	log.Println("context: recovery.recoverSplitCrash, Detected split crash. Truncating index file...")
...
func recoverFreeList(db *DB, usedBlocks []userdblock) error {
	...
	if db.data.size > lastOffset {
		...
		log.Println(lastBlock, db.data.size)
	}
	log.Printf("Recovered freelist. Old len=%d; new len=%d\n", len(db.data.fl.blocks), len(fl.blocks))
	db.data.fl = fl
	return nil
}
...
func (db *DB) recover() error {
	...
	log.Printf("Index file size=%d; data file size=%d\n", db.index.size, db.data.size)
	log.Printf("Header dbInfo %+v\n", db.dbInfo)
