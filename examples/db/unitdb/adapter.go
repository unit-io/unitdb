...
import (
	...
	"github.com/unit-io/unitdb/examples/store"
)

const (
	defaultDatabase = "unitedb"
...
func (a *adapter) Open(jsonconfig string, reset bool) error {
	...
	if err := os.MkdirAll(config.Dir, 0777); err != nil {
		log.Printf("adapter.Open: Unable to create db dir")
	}

	// Attempt to open the database
	a.db, err = unitdb.Open(config.Dir+"/"+defaultDatabase, nil, unitdb.WithMutable())
	if err != nil {
		log.Printf("adapter.Open: Unable to open db")
		return err
	}
